...
// 제외하고 모두 격자 밖으로 쫓겨난다.
public class BOJ_Gold3_19237_어른상어2 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 1; i <= M; i++) {
			int[][] order = new int[5][4];
			
			for (int j = 1; j <= 4; j++) {
				st = new StringTokenizer(in.readLine(), " ");
				order[j][0] = Integer.parseInt(st.nextToken());
				order[j][1] = Integer.parseInt(st.nextToken());
				order[j][2] = Integer.parseInt(st.nextToken());
				order[j][3] = Integer.parseInt(st.nextToken());
			}
			
			sharks[i].dirArr = order;
		}
	...
	private static void moveShark(Shark shark) {
		...
		for(int d : shark.dirArr[shark.dir]) {
			int nr = shark.r + dr[d];
			int nc = shark.c + dc[d];
