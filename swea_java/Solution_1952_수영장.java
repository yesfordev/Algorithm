 ...
 * 문제 유형: dp / 다른 방법으로도 풀어볼 수 있지 않을까?
 * 요구 개념: dp
 * 
 * <풀이법 요약>
 * 1. dp를 이용한 풀이법
...
public class Solution_1952_수영장 {
	...
	static int TC, dayP, monthP, threeMonthP, yearP;
	static int[] use;
	static int[] dp;
	public static void main(String[] args) {
		...
		for (int t = 1; t <= TC; t++) {
			...
			for (int i = 1; i <= 12; i++) {
				int min = Math.min(use[i]*dayP, monthP) + dp[i-1];
				
				if(i>=3) min = Math.min(min, dp[i-3]+threeMonthP);
				
				dp[i] = min;
			}
			
			System.out.print("#"+t+" ");
			if(dp[12]>yearP) System.out.println(yearP);
			else System.out.println(dp[12]);
		}
	}
}
